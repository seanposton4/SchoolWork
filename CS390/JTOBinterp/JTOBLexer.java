...
public class JTOBLexer implements JTOBTokens
{
    //constructor
    public JTOBLexer(final InputStream in) {
        line = "";
        lineNo = 0;
        colNo = 0;
        currentChar = ' ';
    ...
    public void next() {
        // the keyword tokens
        final String[] kw = { "array", "while", "end", "insert", "say", "floop", "am", "if", "else", "to", "false", "true" };
        final int[] kwt = { ARRAY, WHILE, END, INSERT, SAY, FLOOP, AM, IF, ELSE, TO, FALSE, TRUE};
        final char[] c = { '[', ']', '-', '+', '*', '/', '(', ')', '"', ':', ',', '.', '=', '>', '<', '~'}; //Alt + 242 = ≥, Alt + 243 = ≤
        final int[] ct = { LBRACKET, RBRACKET, MINUS, PLUS, MULTIPLY, DIVIDE, LPARENTHESIS, RPARENTHESIS, 
							DOUBLEQUOTES, COLON, EGTHAN, ELTHAN, EQUAL, GTHAN, LTHAN, NEQUAL };
    ...
    public String toString() {
        final String[] label = {  "ENDINPUT", "AM", "ARRAY", "ASSIGN", "CHARACTER", "COLON",  "DIVIDE", "DOUBLEQUOTES", "EGTHAN", "ELSE", "ELTHAN",
								 "EGTHAN", "END", "EQUAL", "FALSE", "FLOOP", "GTHAN", "ID", "IF", "ELSE", "INSERT", "LBRACKET", "LITERAL", "LPARENTHESIS",  
								 "LTHAN", "MINUS", "MULTIPLY", "NEQUAL", "PLUS", "RBRACKET", "RPARENTHESIS", "SAY",  "STRING", "TO", "TRUE", "WHILE", 
								 "error" };
    ...
    public static void main(final String[] args) {
        final JTOBLexer lexer = new JTOBLexer(System.in);

        do {
            if(lexer.getToken() == JTOBLexer.error) {
                lexer.printError("Invalid Token");
            } else {
                System.out.println(lexer);
            }
        } while(lexer.getToken() != JTOBLexer.ENDINPUT);
    }
}
